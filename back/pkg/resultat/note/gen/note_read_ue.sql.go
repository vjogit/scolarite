...
const getUeStats = `-- name: GetUeStats :many
...
matieres_brutes AS (
    ...
    WHERE m.unite_enseignement_id = $1
      AND (
...
type GetUeStatsRow struct {
	UserID               int32    `json:"user_id"`
	FirstName            *string  `json:"firstName"`
	LastName             *string  `json:"lastName"`
	Note                 *float64 `json:"note"`
	AMatiereEliminatoire *bool    `json:"a_matiere_eliminatoire"`
	GradeLettre          string   `json:"grade_lettre"`
	Provenance           string   `json:"provenance"`
}

// 5. Verdict et attribution du Grade
func (q *Queries) GetUeStats(ctx context.Context, uniteEnseignementID int32) ([]GetUeStatsRow, error) {
	rows, err := q.db.Query(ctx, getUeStats, uniteEnseignementID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetUeStatsRow
	for rows.Next() {
		var i GetUeStatsRow
		if err := rows.Scan(
			&i.UserID,
