...
func DeleteImpact(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		services.ServerError(w, r, fmt.Errorf("impact de suppression : lecture des options impossible (ids %v): %w", input.IDs, err))
		return
	}

	impact, err := queries.OptionDeleteImpact(r.Context(), input.IDs)
	if err != nil {
		services.ServerError(w, r, fmt.Errorf("impact de suppression : comptage des options impossible (ids %v): %w", input.IDs, err))
		return
	}
