...
func TestIntegration_UeDeleteImpact(t *testing.T) {
	...
	// Un résultat de jury rattaché à l'UE tombe par cascade : compté, jamais bloquant
	// (le DELETE de l'UE ne bloque pas non plus — défaut consigné, hors lot).
	...
	assert.Empty(t, respJury.Blocking)
}
