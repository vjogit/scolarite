...
func Delete(w http.ResponseWriter, r *http.Request) {
	...
	if jury.RefuserSiJuryDelibere(w, r, jury.PerimetreUE, input.IDs) {
		return
	}
	...
	if _, err := registre.TracerSuppressionEnCascade(r.Context(), tx, "unite_enseignement", input.IDs, services.SubFromCtx(r)); err != nil {
		services.ServerError(w, r, err)
		return
	}

	if err := queries.WithTx(tx).DeleteUniteEnseignement(r.Context(), input.IDs); err != nil {
		services.ServerError(w, r, err)
		return
	}
