...
func DeleteImpact(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		slog.Error("impact de suppression : lecture des périodes impossible", "ids", input.IDs, "error", err)
	...
	if err != nil {
		slog.Error("impact de suppression : comptage des périodes impossible", "ids", input.IDs, "error", err)
		services.InternalServerError(w, r, "Impossible d'évaluer l'impact de la suppression", services.INTERNAL_ERROR, nil)
		return
	}
