...
// des périodes dont les identifiants sont fournis : objets visés, décomptes
...
func DeleteImpact(w http.ResponseWriter, r *http.Request) {
	...
	names, err := queries.FetchPeriodeNamesByIds(r.Context(), input.IDs)
	if err != nil {
		services.ServerError(w, r, fmt.Errorf("impact de suppression : lecture des périodes impossible (ids %v): %w", input.IDs, err))
		return
	}

	impact, err := queries.PeriodeDeleteImpact(r.Context(), input.IDs)
	if err != nil {
		services.ServerError(w, r, fmt.Errorf("impact de suppression : comptage des périodes impossible (ids %v): %w", input.IDs, err))
		return
	}
	...
	// Syllabus (correction A1) : les liaisons UE ↔ compétence des UE du
	// sous-arbre tombent avec elles.
