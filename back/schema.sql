...
CREATE FUNCTION public.syllabus_empreinte(VARIADIC textes text[]) RETURNS text
...
CREATE TABLE public.bloc_competence (
    ...
    CONSTRAINT chk_bloc_competence_libelle_length CHECK ((length(libelle) > 0)),
    CONSTRAINT chk_bloc_competence_ordre_positive CHECK ((ordre > 0))
);



ALTER TABLE public.bloc_competence ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME public.bloc_competence_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE public.competence (
    ...
    contexte text,
    finalites text,
    CONSTRAINT chk_competence_action_length CHECK ((length(action) > 0)),
    CONSTRAINT chk_competence_ordre_positive CHECK ((ordre > 0))
);
...
CREATE TABLE public.syllabus_matiere (
    id integer NOT NULL,
    version integer DEFAULT 1 NOT NULL,
    matiere_id integer NOT NULL,
    contexte text,
    ...
    plan_cours text,
    ressources text,
    dimension_socio_env text,
    heures_cours numeric(5,2),
    heures_cours_td numeric(5,2),
    heures_td numeric(5,2),
    heures_tp numeric(5,2),
    heures_projet numeric(5,2),
    heures_autonomie numeric(5,2),
    heures_controle numeric(5,2),
    heures_perso numeric(5,2),
    responsable_id integer,
    CONSTRAINT chk_syllabus_matiere_heures_autonomie_positive CHECK ((heures_autonomie >= (0)::numeric)),
    CONSTRAINT chk_syllabus_matiere_heures_controle_positive CHECK ((heures_controle >= (0)::numeric)),
    CONSTRAINT chk_syllabus_matiere_heures_cours_positive CHECK ((heures_cours >= (0)::numeric)),
    CONSTRAINT chk_syllabus_matiere_heures_cours_td_positive CHECK ((heures_cours_td >= (0)::numeric)),
    CONSTRAINT chk_syllabus_matiere_heures_perso_positive CHECK ((heures_perso >= (0)::numeric)),
    CONSTRAINT chk_syllabus_matiere_heures_projet_positive CHECK ((heures_projet >= (0)::numeric)),
    CONSTRAINT chk_syllabus_matiere_heures_td_positive CHECK ((heures_td >= (0)::numeric)),
    CONSTRAINT chk_syllabus_matiere_heures_tp_positive CHECK ((heures_tp >= (0)::numeric))
);



ALTER TABLE public.syllabus_matiere ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME public.syllabus_matiere_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE VIEW public.syllabus_matiere_source AS
 SELECT matiere_id,
    version,
    contexte,
    objectifs,
    ...
    ressources,
    dimension_socio_env,
    public.syllabus_empreinte(VARIADIC ARRAY[contexte, objectifs, prerequis, activites, evaluation, plan_cours, ressources, dimension_socio_env]) AS empreinte
   FROM public.syllabus_matiere;



CREATE TABLE public.syllabus_matiere_traduction (
    matiere_id integer NOT NULL,
    ...
    ressources text,
    dimension_socio_env text,
    version_source integer NOT NULL,
    empreinte_source character varying(64) NOT NULL,
    statut character varying(12) NOT NULL,
    modele text,
    traduit_le timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_syllabus_matiere_traduction_langue CHECK ((((langue)::text ~ '^[a-z]{2}$'::text) AND ((langue)::text <> 'fr'::text))),
    CONSTRAINT chk_syllabus_matiere_traduction_statut CHECK (((statut)::text = ANY ((ARRAY['automatique'::character varying, 'relue'::character varying])::text[])))
);
...
CREATE TABLE public.ue_competence (
    ue_id integer NOT NULL,
    competence_id integer NOT NULL,
    enseignee boolean DEFAULT false NOT NULL,
    mise_en_oeuvre boolean DEFAULT false NOT NULL,
    evaluee boolean DEFAULT false NOT NULL,
    CONSTRAINT chk_ue_competence_au_moins_un CHECK ((enseignee OR mise_en_oeuvre OR evaluee))
);
...
CREATE VIEW public.unite_enseignement_source AS
 SELECT id AS ue_id,
    version,
    description,
    public.syllabus_empreinte(VARIADIC ARRAY[description]) AS empreinte
   FROM public.unite_enseignement;



CREATE TABLE public.unite_enseignement_traduction (
    ue_id integer NOT NULL,
    langue character varying(2) NOT NULL,
    version integer DEFAULT 1 NOT NULL,
    description text,
    ...
    modele text,
    traduit_le timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_unite_enseignement_traduction_langue CHECK ((((langue)::text ~ '^[a-z]{2}$'::text) AND ((langue)::text <> 'fr'::text))),
    CONSTRAINT chk_unite_enseignement_traduction_statut CHECK (((statut)::text = ANY ((ARRAY['automatique'::character varying, 'relue'::character varying])::text[])))
);
...
ALTER TABLE ONLY public.bloc_competence
    ADD CONSTRAINT bloc_competence_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.syllabus_matiere_traduction
    ADD CONSTRAINT pk_syllabus_matiere_traduction PRIMARY KEY (matiere_id, langue);



ALTER TABLE ONLY public.ue_competence
    ADD CONSTRAINT pk_ue_competence PRIMARY KEY (ue_id, competence_id);



ALTER TABLE ONLY public.unite_enseignement_traduction
    ADD CONSTRAINT pk_unite_enseignement_traduction PRIMARY KEY (ue_id, langue);
...
ALTER TABLE ONLY public.syllabus_matiere
    ADD CONSTRAINT syllabus_matiere_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY public.competence
    ADD CONSTRAINT uk_competence_ordre UNIQUE (bloc_id, ordre);
...
ALTER TABLE ONLY public.syllabus_matiere
    ADD CONSTRAINT uk_syllabus_matiere_matiere UNIQUE (matiere_id);
...
CREATE INDEX idx_competence_bloc ON public.competence USING btree (bloc_id);
...
CREATE INDEX idx_ue_competence_competence ON public.ue_competence USING btree (competence_id);
...
ALTER TABLE ONLY public.competence
    ADD CONSTRAINT fk_competence_bloc FOREIGN KEY (bloc_id) REFERENCES public.bloc_competence(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.syllabus_matiere
    ADD CONSTRAINT fk_syllabus_matiere_matiere FOREIGN KEY (matiere_id) REFERENCES public.matiere(id) ON DELETE CASCADE;



ALTER TABLE ONLY public.syllabus_matiere
    ADD CONSTRAINT fk_syllabus_matiere_responsable FOREIGN KEY (responsable_id) REFERENCES public."user"(id) ON DELETE SET NULL;



ALTER TABLE ONLY public.syllabus_matiere_traduction
    ADD CONSTRAINT fk_syllabus_matiere_traduction_fiche FOREIGN KEY (matiere_id) REFERENCES public.syllabus_matiere(matiere_id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.ue_competence
    ADD CONSTRAINT fk_ue_competence_competence FOREIGN KEY (competence_id) REFERENCES public.competence(id) ON DELETE CASCADE;



ALTER TABLE ONLY public.ue_competence
    ADD CONSTRAINT fk_ue_competence_ue FOREIGN KEY (ue_id) REFERENCES public.unite_enseignement(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.unite_enseignement
    ADD CONSTRAINT fk_ue_responsable FOREIGN KEY (responsable_id) REFERENCES public."user"(id) ON DELETE SET NULL;



ALTER TABLE ONLY public.unite_enseignement_traduction
    ADD CONSTRAINT fk_unite_enseignement_traduction_ue FOREIGN KEY (ue_id) REFERENCES public.unite_enseignement(id) ON DELETE CASCADE;
